...
// TestOpponentRecordsByeCategories verifies that the canonical per-round
// record preserves the C.07 category, awarded points, and VUR status.
func TestOpponentRecordsByeCategories(t *testing.T) {
	for _, bt := range allByeTypes() {
		t.Run(bt.String(), func(t *testing.T) {
			state := stateWithSingleBye(bt)
			table := buildOpponentRecords(state, scoresFor("p1", "p2"))
			...
			if record.Category != wantCategory || record.Points != wantPoints || record.IsVUR != wantVUR {
				t.Errorf("record = %+v, want category=%v points=%v VUR=%v", record, wantCategory, wantPoints, wantVUR)
			}
		})
	}
}

// TestOpponentRecordsPlayedMeansOverTheBoard verifies that no bye is marked
// Played; the field has the narrower Article 15 game-over-the-board meaning.
func TestOpponentRecordsPlayedMeansOverTheBoard(t *testing.T) {
	for _, byeType := range allByeTypes() {
		t.Run(byeType.String(), func(t *testing.T) {
			table := buildOpponentRecords(stateWithSingleBye(byeType), scoresFor("p1", "p2"))
			if table.records["p1"][0].Played {
				t.Errorf("bye %v is marked as played over the board", byeType)
			}
		})
	}
}
