...
func TestAllocateColor_BothNoPreference(t *testing.T) {
	...
	white := &PlayerState{ID: "p1", TPN: 1, PairingNumber: 1}
	black := &PlayerState{ID: "p2", TPN: 2, PairingNumber: 2}
	wID, bID := AllocateColor(white, black, false, 1, nil, AlternateByBoard)
	if wID != "p1" || bID != "p2" {
...
func TestAllocateColor_BothAbsoluteOpposite(t *testing.T) {
	...
	wID, bID := AllocateColor(a, b, false, 1, nil, AlternateByBoard)
	if wID != "p1" || bID != "p2" {
...
func TestAllocateColor_NoPreference_EvenBoard(t *testing.T) {
	...
	a := &PlayerState{ID: "p1", TPN: 1, PairingNumber: 1}
	b := &PlayerState{ID: "p2", TPN: 2, PairingNumber: 2}
	wID, bID := AllocateColor(a, b, false, 2, nil, AlternateByBoard)
	if wID != "p2" || bID != "p1" {
...
func TestAllocateColor_NoPreference_AlternatesByBoard(t *testing.T) {
	...
	w, bl := AllocateColor(a, b, false, 1, nil, AlternateByBoard)
	if w != "p1" || bl != "p2" {
		t.Errorf("board 1: expected p1-p2, got %s-%s", w, bl)
	}
	// Board 2 (even): lower-ranked white
	w, bl = AllocateColor(a, b, false, 2, nil, AlternateByBoard)
	if w != "p2" || bl != "p1" {
		t.Errorf("board 2: expected p2-p1, got %s-%s", w, bl)
	}
	// Board 3 (odd): higher-ranked white
	w, bl = AllocateColor(a, b, false, 3, nil, AlternateByBoard)
	if w != "p1" || bl != "p2" {
		t.Errorf("board 3: expected p1-p2, got %s-%s", w, bl)
	}
	// Board 4 (even): lower-ranked white
	w, bl = AllocateColor(a, b, false, 4, nil, AlternateByBoard)
	if w != "p2" || bl != "p1" {
		t.Errorf("board 4: expected p2-p1, got %s-%s", w, bl)
	}
}

func TestAllocateColor_NoPreference_FixedNumberParity(t *testing.T) {
	a := &PlayerState{ID: "a", PairingNumber: 2, TPN: 1}
	b := &PlayerState{ID: "b", PairingNumber: 3, TPN: 2}

	whiteID, blackID := AllocateColor(a, b, false, 1, nil, FixedNumberParity)
	if whiteID != "b" || blackID != "a" {
		t.Errorf("even-numbered higher-ranked player: got %s-%s, want b-a", whiteID, blackID)
	}

	whiteID, blackID = AllocateColor(a, b, false, 2, nil, FixedNumberParity)
	if whiteID != "b" || blackID != "a" {
		t.Errorf("board number changed fixed parity: got %s-%s, want b-a", whiteID, blackID)
	}

	initialBlack := ColorBlack
	whiteID, blackID = AllocateColor(a, b, false, 1, &initialBlack, FixedNumberParity)
	if whiteID != "a" || blackID != "b" {
		t.Errorf("black initial colour: got %s-%s, want a-b", whiteID, blackID)
	}
}

func TestAllocateColor_TopSeedBlack_Board1(t *testing.T) {
	a := &PlayerState{ID: "p1", TPN: 1, PairingNumber: 1}
	b := &PlayerState{ID: "p4", TPN: 4, PairingNumber: 4}
	black := ColorBlack

	whiteID, blackID := AllocateColor(a, b, false, 1, &black, AlternateByBoard)
	if blackID != "p1" {
...
func TestAllocateColor_TopSeedBlack_Board2(t *testing.T) {
	a := &PlayerState{ID: "p2", TPN: 2, PairingNumber: 2}
	b := &PlayerState{ID: "p3", TPN: 3, PairingNumber: 3}
	black := ColorBlack

	whiteID, blackID := AllocateColor(a, b, false, 2, &black, AlternateByBoard)
	if blackID != "p3" {
...
func TestAllocateColor_TopSeedNil_DefaultBehavior(t *testing.T) {
	...
	whiteID, _ := AllocateColor(a, b, false, 1, nil, AlternateByBoard)
	if whiteID != "p1" {
...
func TestAllocateColor_TopSeedWhite_SameAsDefault(t *testing.T) {
	...
	whiteID, _ := AllocateColor(a, b, false, 1, &white, AlternateByBoard)
	if whiteID != "p1" {
